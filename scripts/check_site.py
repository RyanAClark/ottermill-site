...
def _internal_target(
    ...
    if not parsed.netloc and not scheme and not url_path:
        if source.is_file():
    ...
    if "\\" in decoded_path:
        return True, None, f'internal reference uses a backslash: "{reference}"'
    ...
    if not target.is_relative_to(resolved_root):
        return True, None, f'internal reference escapes the site root: "{reference}"'
...
def scan_site(root: Path) -> ScanResult:
    ...
    for reference in references:
        is_internal, _target, error = _internal_target(
